...
//    unsigned edge_cb_id = 0;
    ...
    if((rv = set_mode(p->pi, (unsigned) PPS_GPIO_PIN, PI_INPUT) == 0))
    {
        ...
        fflush(OUTPUT_PRINT);
#endif
        if((p->edge_cb_id = callback(p->pi, (unsigned) PPS_GPIO_PIN, RISING_EDGE, (CBFunc_t) onEdge)) != 0)                   // RISING_EDGE, FALLING_EDGE, or EITHER_EDGE
        {
            switch(p->edge_cb_id)
            {
    ...
    //-----------------------------------------
    // if((rv = set_mode(p->pi, (unsigned) PPS_GPIO_PIN, PI_INPUT) == 0))
    if((rv = gpio_claim_input(p->pi, p->ppsHandle, 0, PPS_GPIO_PIN) == 0))    // open GPIO 27for input
    {
#if _DEBUG
        fprintf(OUTPUT_PRINT, "    [CHILD] Before setting up callback to: onEdge()...\n");
        fflush(OUTPUT_PRINT);
        ...
        if((p->edge_cb_id = callback(p->pi, p->ppsHandle, (unsigned) PPS_GPIO_PIN, RISING_EDGE, (CBFunc_t) onEdge, NULL)) != 0)                   // RISING_EDGE, FALLING_EDGE, or EITHER_EDGE
        {
            switch(p->edge_cb_id)
            {
...
int initGPIO(volatile ctlList *p)
{
    ...
    //-----------------------------------------
#if(USE_NOPI)
    if(pololu_i2c_init(adapter) >= 0)
#elif(USE_RGPIO)
    if((p->pi = rgpiod_start(NULL, NULL)) >= 0)
